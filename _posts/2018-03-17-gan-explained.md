...
> 이 글은 [마이크로소프트웨어 391호 인공지능의 체크포인트(THE CHECKPOINT OF AI)](https://www.imaso.co.kr/archives/1301)에 '쉽게 쓰이는 GAN'이라는 제목으로 기고된 글입니다. 블로그에는 이 글의 원제인 '쉽게 씌어진 GAN'으로 포스팅합니다. 이 제목은 윤동주 시인의 ['쉽게 씌어진 시'](https://namu.wiki/w/%EC%89%BD%EA%B2%8C%20%EC%94%8C%EC%96%B4%EC%A7%84%20%EC%8B%9C)에 모티브를 얻어 지어졌습니다.
...
먼저구분자를 학습시켜보자. 구분자는 진짜 이미지를 입력하면 1에 가까운 확률값을 출력하고, 가짜 데이터를 입력하면 0에 가까운 확률값을 출력해야 한다.따라서 구분자의 손실 함수는 두 가지의 합으로 이루어진다. 진짜 이미지를 입력했을 때의 출력값과 1과의 차이, 그리고 가짜 이미지를 입력했을 때의출력값과 0과의 차이, 두 경우의 합이 구분자의 손실 함수다. 이 손실 함수의 값을 최소화하는 방향으로 구분자의 매개 변수가 업데이트된다. 
